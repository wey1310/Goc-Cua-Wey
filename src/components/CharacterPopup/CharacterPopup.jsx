...
function CharacterPopup({
    ...
    const [likes, setLikes] = useState(character?.leafCount ?? 0);
...
// ================= Display =================
    ...
    const openLink = async (
        ...
        if (type === "ggai") {
            try {
                await increaseGGAIClick(character.id, character.name);
                onTeaIncrease?.();
            } catch (error) {
                console.error("Update ggai:", error);
            }
        } else if (type === "plot") {
            try {
                await increasePlotClick(character.id, character.name);
                onTeaIncrease?.();
            } catch (error) {
                console.error("Update plot:", error);
            }
        }
    ...
    />
                ...
                <div className={styles.cupsWrapper}>
                    ...
                    <div
                        ...
                        onClick={() => openLink(ggaiLink, "ggai")}
                        title={
                            hasGGAI
                                ? "Thưởng trà GGAI"
                                : "Chưa có GGAI"
                    ...
                    <div
                        ...
                        onClick={() => openLink(plotLink, "plot")}
                        title={
                            hasPlot
                                ? "Thưởng trà Plot"
                                : "Chưa có Plot"
